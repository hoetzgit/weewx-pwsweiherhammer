...
def convert_pollutant_units(pollutant, obs_value, obs_unit, required_unit, temp_in_kelvin, pressure_in_kilopascals):
    ...
    if obs_unit.startswith('parts_per_') and required_unit.endswith('_per_meter_cubed'):
        ppb = obs_value
        if obs_unit == 'parts_per_million':
            ppb = weewx.units.conversionDict[obs_unit]['parts_per_billion'](obs_value)
    ...
    elif obs_unit.endswith('_per_meter_cubed') and required_unit.startswith('parts_per_'):
        ug_per_m3 = obs_value
        ...
        if required_unit == 'parts_per_billion':
            return ug_per_m3
        else:
            return weewx.units.conversionDict['parts_per_billion'][required_unit](ppb)
...
weewx.units.conversionDict['parts_per_billion'] = { 'parts_per_million': lambda x: x * 1000.0 }
weewx.units.conversionDict['parts_per_million'] = { 'parts_per_billion': lambda x: x / 1000.0 }
