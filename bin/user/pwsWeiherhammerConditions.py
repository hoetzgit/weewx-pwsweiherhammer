...
try:
    ...
    def logmsg(level, msg):
        syslog.syslog(level, 'pwsWeiherhammerConditions: %s' % msg)
...
class ThunderstormDuration(StdService):
    ...
    def newLoopPacket(self, event):
        ...
        if self.debug:
            logdbg("Loop Thunderstorm: Thunderstorm=%d" % (thunderstorm))
            logdbg("Loop Thunderstorm: Avg10: %0.2f" % (avgDeque(self.thunderstorm_10)))
            logdbg("Loop Thunderstorm: Avg20: %0.2f" % (avgDeque(self.thunderstorm_20)))
            logdbg("Loop Thunderstorm: Avg30: %0.2f" % (avgDeque(self.thunderstorm_30)))
            logdbg("Loop Thunderstorm: Avg60: %0.2f" % (avgDeque(self.thunderstorm_60)))
    ...
    def newArchiveRecord(self, event):
        ...
        if self.debug:
            logdbg("Archive Thunderstorm: Avg10: %0.2f" % (event.record['thunderstorm_avg10m']))
            logdbg("Archive Thunderstorm: Avg20: %0.2f" % (event.record['thunderstorm_avg20m']))
            logdbg("Archive Thunderstorm: Avg30: %0.2f" % (event.record['thunderstorm_avg30m']))
            logdbg("Archive Thunderstorm: Avg60: %0.2f" % (event.record['thunderstorm_avg60m']))
...
class RainDuration(StdService):
    ...
    def newLoopPacket(self, event):
        ...
        if self.debug:
            logdbg("Loop Rain: Raining=%d" % (raining))
            logdbg("Loop Rain: Avg10: %0.2f" % (avgDeque(self.rain_10)))
            logdbg("Loop Rain: Avg20: %0.2f" % (avgDeque(self.rain_20)))
            logdbg("Loop Rain: Avg30: %0.2f" % (avgDeque(self.rain_30)))
            logdbg("Loop Rain: Avg60: %0.2f" % (avgDeque(self.rain_60)))
    ...
    def newArchiveRecord(self, event):
        ...
        if self.debug:
            logdbg("Archive Rain: Avg10: %0.2f" % (event.record['rain_avg10m']))
            logdbg("Archive Rain: Avg20: %0.2f" % (event.record['rain_avg20m']))
            logdbg("Archive Rain: Avg30: %0.2f" % (event.record['rain_avg30m']))
            logdbg("Archive Rain: Avg60: %0.2f" % (event.record['rain_avg60m']))
