...
class OpenWeatherMap(weewx.restx.StdRESTful):
    def __init__(self, engine, config_dict):
        ...
        enable: Service is enabled/disabled
        Default is True
        """
        ...
        try:
            if not to_bool(site_dict['enable']):
                log.info("Data will not be posted: Service not enabled.")
                return None
        except KeyError:
            pass
        ...
        loginf("Data will be uploaded for station_id %s" % site_dict['station_id'])
...
class OpenWeatherMapThread(weewx.restx.RESTThread):
    ...
    _SERVER_URL = 'https://api.openweathermap.org/data/3.0/measurements'
    _DATA_MAP = {
