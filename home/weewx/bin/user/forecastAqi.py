...
class AerisAqiMonitor(StdService):
    ...
    def __init__(self, engine, config_dict):
        ...
        if dbcol != memcol:
            raise Exception('AerisAqiMonitor schema mismatch: %s != %s' % (dbcol, memcol))
    ...
    def new_archive_record(self, event):
        ...
        latitude = 49.632270           # d.get('latitude', None)
        longitude = 12.056186          # d.get('longitude', None)
        aqiIndex_stale_timer =  3540   #self.skin_dict['Extras']['aqiIndex_stale']
        aqiIndex_url = "https://api.aerisapi.com/airquality/%s,%s?&format=json&client_id=%s&client_secret=%s" % (latitude, longitude, self.aeris_id_key, self.aeris_secret_key)
        ...
        if aqiIndex_is_stale:
            ...
            log.info("AerisAqiMonitor New AerisWeather ForecastAQI data downloaded to %s", aqiIndex_file)

        """save data to database """
        now = int(time.time() + 0.5)
        delta = now - event.record['dateTime']
        if delta > event.record['interval'] * 60:
            log.debug("AerisAqiMonitor Skipping record: time difference %s too big", delta)
            return
    ...
    def get_data(self, now_ts, last_ts):
        ...
        try:
            ...
            log.debug("AerisAqiMonitor read failed for AqiIndex: %s",  e)
