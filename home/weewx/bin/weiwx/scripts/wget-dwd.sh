...
# \n" >"${TMP}"
    ...
    if [ "$?" -eq 0 ]; then
        for SKIN in ${SKINS[@]}; do
            ${HTML2ENT} <"${TMP}" >"${SKIN}/${FN}.html"
            ${TOUCH} "${TMP}" "${SKIN}/${FN}.html"
        done
        for FORECAST in ${FORECASTS[@]}; do
            ${HTML2ENT} <"${TMP}" >"${FORECAST}/${FN}.html"
            ${TOUCH} "${TMP}" "${FORECAST}/${FN}.html"
        done
        ${RM} "${TMP}"
    fi
done
...
for SKIN in ${SKINS[@]}; do
    ${CP} "/tmp/${DWD_MAP##*/}" ${SKIN}/
    ${CP} "/tmp/${DWD_MAP2##*/}" ${SKIN}/
    ${CP} "/tmp/${DWD_WARN##*/}" ${SKIN}/
    ${CP} "/tmp/${DWD_WARNJ##*/}" ${SKIN}/
done
for WARNING in ${WARNINGS[@]}; do
    ${CP} "/tmp/${DWD_MAP##*/}" ${WARNING}/
    ${CP} "/tmp/${DWD_MAP2##*/}" ${WARNING}/
    ${CP} "/tmp/${DWD_WARN##*/}" ${WARNING}/
    ${CP} "/tmp/${DWD_WARNJ##*/}" ${WARNING}/
done
${RM} "/tmp/${DWD_MAP##*/}"
${RM} "/tmp/${DWD_MAP2##*/}"
${RM} "/tmp/${DWD_WARN##*/}"
